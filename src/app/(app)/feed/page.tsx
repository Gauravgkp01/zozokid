...
import { intelligentSafeFeedCuration } from '@/ai/flows/intelligent-safe-feed-curation-flow';
import { Badge } from '@/components/ui/badge';

export default async function FeedPage() {
  const aiResponse = await intelligentSafeFeedCuration({
    childAge: 8,
    childInterests: ['minecraft', 'lego', 'drawing'],
    excludedChannels: ['Cocomelon'],
    excludedKeywords: ['scary', 'horror'],
  });
  ...
  return (
    <div className="container mx-auto flex h-[calc(100vh-5rem)] flex-col items-center justify-center gap-6">
      <div className="w-full max-w-md rounded-lg border bg-card p-4 text-center">
        <h2 className="font-headline text-lg font-semibold">
          AI-Powered Feed Curation
        </h2>
        <p className="mb-3 text-sm text-muted-foreground">
          {aiResponse.curationRationale}
        </p>
        <div className="flex flex-wrap justify-center gap-2">
          {aiResponse.youtubeSearchQueries.map((q) => (
